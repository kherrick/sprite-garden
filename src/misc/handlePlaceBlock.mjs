...
export async function handlePlaceBlock(
  ...
  const shadow = getShadowRoot(globalThis.document, "sprite-garden");
  ...
  if (materialsInventory[selectedMaterialType] <= 0) {
    const message = `No ${selectedMaterialType.toLowerCase()} available to place`;
  ...
  if (!tileToPlace) {
    const message = `Invalid material type: ${selectedMaterialType.toLowerCase()}`;
  ...
  const message = `Placed ${selectedMaterialType.toLowerCase()} at (${targetX}, ${targetY}), ${materialsInventory[selectedMaterialType] - 1} remaining.`;
