...
import { getTileNameById } from "../state/config/tiles.mjs";
...
function handleBreakBlock(
  ...
  if (blocksToBreak.length > 0) {
    ...
    const blockNames = blocksToBreak
      .map((block) => getTileNameById(tiles, block.tile.id).toLowerCase())
      .join(", ");

    getShadowRoot(globalThis.document, "sprite-garden").dispatchEvent(
